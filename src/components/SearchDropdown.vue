<template>
  <div class="z-50 transition-height duration-100 shadow-lg w-full p-3 pt-4 laptop:mt-2 desktop:mt-3 bg-white rounded-lgg absolute overflow-x-hidden" :class="show_add_btn ? 'h-48' : 'h-96'">
    <div class="p-2 overflow-x-hidden thin-scrollbar flex flex-col gap-2 dropdown-scroll-container pr-4">
      ...
      <SearchItemAddButton @clicked="addNewItemClicked" v-show="showAddBtn && show_add_btn" :title="searchText"/>
...
const show_add_btn = computed(() => props.data.length == 0 && props.searchText != '' ? true : false)
